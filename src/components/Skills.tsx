      ...
      <div className="flex flex-wrap justify-center gap-6">
        {skillCategories.map((cat, i) => (
          <motion.div
            ...
            className="group relative overflow-hidden rounded-2xl border border-border/30 bg-surface-light/50 p-6 transition-all hover:border-border/60 hover:shadow-xl hover:shadow-black/5 w-full sm:w-[calc(50%-12px)] lg:w-[calc(33.333%-16px)]"
          >
            ...
            {/* Icon */}
            <div className={`mb-5 inline-flex items-center justify-center rounded-xl bg-gradient-to-br ${cat.color} p-3 text-white`}>
              {cat.icon}
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {cat.skills.map((skill) => (
                <span
                  key={skill}
                  className="rounded-full border border-border/30 px-3 py-1 text-xs text-muted transition-colors group-hover:border-border/50 group-hover:text-foreground/70"
                >
                  {skill}
                </span>
              ))}
